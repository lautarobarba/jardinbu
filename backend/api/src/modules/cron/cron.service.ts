...
export class CronService {
  ...
  async handleTestCron(job: Job) {
    ...
    this._logger.debug(
      "Ejecutando tarea enviada a la cola handleTestCron por testCron"
    );
    const { msg } = job.data;
  }

  // Periodo: cada 24HS
  // Tarea: Eliminar imagenes que no están en uso
  // Envía la tarea 'handleDeleteUselessImages' a la cola
  @Cron(CronExpression.EVERY_DAY_AT_1AM, {
    timeZone: "America/Argentina/Buenos_Aires",
  })
  async deleteUselessImages() {
    this._logger.debug("deleteUselessImages()");
    const job: Job = await this._cronQueue.add("handleDeleteUselessImages");
  ...
  // Ejecula la próxima tarea 'handleDeleteUselessImages' de la cola
  @Process("handleDeleteUselessImages")
  async handleDeleteUselessImages(job: Job) {
    this._logger.debug("handleDeleteUselessImages()");
    await this._imageService.deleteUselessImages();
  }
}
